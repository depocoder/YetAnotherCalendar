...
def get_program_ids(session: requests.Session):
    response = session.get('https://netology.ru/backend/api/user/programs/calendar/filters',)
    response.raise_for_status()
...
def get_calendar(session: requests.Session, calendar_id):
    response = session.get('https://netology.ru/backend/api/user/programs/calendar',
